...
source("../data/OR3_NBDA code 1.2.15.r") # sources functions
  ...
  for (e in cutoff_vector){ ## run loop for each value of the cutoff vector (e refers to the position of the value in the vector)
    ...
    res2.p <- foreach(i=1:cores, .combine='rbind') %dopar%
      { 
        ...
        source("../data/OR3_NBDA code 1.2.15.r")
...
# Read in files
gxi <- readRDS("../data/gxi.RData")

together_apart_output <- readRDS("../data/together_apart_output.RData")
together_matrix <- read.csv("../data/together_matrix.csv")
apart_matrix <- read.csv("../data/apart_matrix.csv")
...
save(sim1, file="sensitivity_NBDA_with_error_false neg_drop learners.RData") # save the sim1 object 
